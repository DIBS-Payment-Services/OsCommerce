...
class dibsflex_api extends dibsflex_helpers {
    ...
    function dibsflex_api_applyFWInvoice(&$aData, $oOrderInfo) {
		...
		$aData ['ordline'.$i.'-2'] = $oItem->name;
		$aData ['ordline'.$i.'-3'] = $oItem->sku;
		$aData ['ordline'.$i.'-4'] = round($oItem->price / 100, 2);
		$aData ['ordline'.$i.'-5'] = round($oItem->tax_rate / 100, 2);
		$aData ['ordline'.$i.'-6'] = round($oItem->qty);
		$aData ['ordline'.$i.'-7'] = round($oItem->qty * ($oItem->price + 
                                                   $oItem->tax_rate) / 100, 2);
                $i++;
            }
    ...
    function dibsflex_api_getInvoiceXML($oOrderInfo) {
        $VerifiedOrderSum = $oOrderInfo->order->total;
        $Delta1 = 0;
        $Delta2 = 0;
        $Delta3 = 0;
        $Delta4 = 0;
        ...
        foreach($oOrderInfo->items as $oItem) {
            ...
            $sDivisor = $oItem->price * round($oItem->qty, 2);
            if($sDivisor == 0) {
                $sTmpTax = 0;
            }
            else {
                $sTmpTax = round(($oItem->tax_rate * 100) / 
                                 $sDivisor, 2) * 100;
            }
            
            if(!empty($oItem->name)) $sTmpName = $oItem->name;
            elseif(!empty($oItem->sku)) $sTmpName = $oItem->sku;
            ...
            $Delta1 = round($oItem->qty) * $oItem->price;
            $VerifiedOrderSum = $VerifiedOrderSum - $Delta1;
		 
            $Delta2 = $oItem->tax_rate;
            $VerifiedOrderSum =  $VerifiedOrderSum - $Delta2;
        
            unset($sTmpTax, $sDivisor, $sTmpName);
        ...
        if($oOrderInfo->shipping->rate == 0) {
            $sTmpShippingTax = 0;
        }
        else {
            $sTmpShippingTax = round(($oOrderInfo->shipping->tax * 100) / 
                                      $oOrderInfo->shipping->rate, 2) * 100;
        }
        
        $aAttributs = array('itemID' => 'ShippingCost',
                            'itemDescription' => $oOrderInfo->shipping->method,
                            'orderRowNumber' => $i,
                            'quantity' => 1,
                            'price' => $oOrderInfo->shipping->rate,
                            'unitCode' => 'pcs',
                            'VATPercent' => $sTmpShippingTax);
        ...
        $Delta4 = $oOrderInfo->shipping->tax;
        $VerifiedOrderSum =  $VerifiedOrderSum - $Delta4;

        unset($sTmpShippingTax);
        ...
        if ($VerifiedOrderSum >= 0.01 || $VerifiedOrderSum <= -0.01) {
            ...
            $i++;
            $aAttributs = array('itemID' => 'RoundingErrComp',
                                'itemDescription' => 'Compensation For Rounding Errors',
                                'orderRowNumber' => $i,
                                'quantity' => 1,
                                'price' => round($VerifiedOrderSum,2),
                                'unitCode' => 'pcs',
                                'VATPercent' => '0');
